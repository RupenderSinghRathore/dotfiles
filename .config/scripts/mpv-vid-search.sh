...
# Find files
mapfile -t files < <(find "${search_dirs[@]}" -type f \( -iname '*.mkv' -o -iname '*.mp4' -o -iname '*.webm' \) -print 2>/dev/null)
...
chosen_short=$(
    printf '%s\n' "${display_list[@]}" |
        ...
        rofi -dmenu -i -p "Select file:" -theme "$HOME/dotfiles/.config/rofi/config-mpv.rasi"
)
